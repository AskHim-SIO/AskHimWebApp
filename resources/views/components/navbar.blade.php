<nav class="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-800">
    <div class="container flex flex-wrap justify-between items-center mx-auto">
        ...
        <div class="flex items-center md:order-2">
            @if(\Illuminate\Support\Facades\Cookie::has('guid'))
                <button type="button"
                        class="flex mr-3 text-sm bg-gray-800 rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600"
                        id="user-menu-button" aria-expanded="false" data-dropdown-toggle="dropdown">
                    <span class="sr-only">Open user menu</span>
                    <img class="w-8 h-8 rounded-full" src="{{asset('assets/img/image.png')}}"
                         alt="user photo">
                </button>
            @else
                <a href="{{route('login')}}">
            ...
            @endif
                <div
                    ...
                    <div class="py-3 px-4">
                            <span
                                class="block text-sm text-gray-900 dark:text-white">@php echo strip_tags(session('name')) @endphp</span>
                        <span
                            class="block text-sm font-medium text-gray-500 truncate dark:text-gray-400">@php echo strip_tags(session('firstname')) @endphp</span>
                    </div>
