            ...
            >
              <div class="h-20 w-20 rounded-full border overflow-hidden">
                ...
                <img
                    ...
                    alt="Avatar"
                    class="h-full w-full"
                    src="http://192.168.10.11/cdn/userPicture.png"
                />
              </div>
              ...
              <div class="flex flex-col h-full overflow-x-auto mb-4 pt-5">
                <div class="flex flex-col  h-full">
                  <div v-for="message in messages" v-bind:key="message.id" class="grid grid-cols-12 gap-y-2">
                    <!-- messages reception -->
                    <div v-on:click="goProfil(message.author.id)" v-if="message.author.id !== id" class="col-start-1 mouse-pointer col-end-8 p-3 rounded-lg">
...
export default {
  ...
  methods: {
    ...
    finaliser() {
      axios.put(`http://192.168.50.11:4001/service/validate-service?serviceId=${this.lastChat.Service.id}&userId=${this.id}`)
          .then((res) => {
            if (res.data.success) {
    ...
    loadChat(chat) {
      ...
      axios.get(`http://192.168.50.11:4001/service/get-services-from-user-by-token/${this.guid}`)
          .then(res => {
      ...
      axios.get(`http://192.168.50.11:4001/chat/get-discussion-by-id/${chat.Uuid}`)
          .then(res => {
      ...
      axios.put(`http://192.168.50.11:4001/chat/read-messages?discussionId=${chat.Uuid}&userToken=${this.guid}`)
    ...
    sendMessage(e) {
      if (this.discussionToken !== 0 && this.guid && this.writeMessage !== "") {
        axios.post(`http://192.168.50.11:4001/chat/post-message?discussionId=${this.discussionToken}&userToken=${this.guid}&message=${this.writeMessage}`)
            .then(res => {
              if (res.data.success) {
    ...
    reloadConv() {
      ...
      axios.get(`http://192.168.50.11:4001/user/get-user-by-token/${this.guid}`)
          .then(res => {
            this.id = res.data.id
            axios.get(`http://192.168.50.11:4001/chat/get-discussions-from-user-by-token/${this.guid}`)
                .then(res => {
                  this.utilisateurs = []
